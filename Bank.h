...
private:
	...
	*/
	...
	{	
		...
		for (int i = 0; i < customers.size(); i++)
		{
			if (customers[i]->getName() == cust->getName())
				break;
			else if ((i == customers.size() + 1) && (customers[i]->getName() != cust->getName()))
				return acct;
		...
		{
			account_id = 100 + accounts.size(); ///all savings accounts will be in the 100 range
		}
		else
			account_id = 200 + accounts.size(); /// all checking accounts will be in the 200 range
		acct = new Account(cust, account_id);
...
public:
	...
	*/
	...
	{	
		...
		Customer *cust = new Customer(name, address, telephone, age, cust_type, account_type, customer_id);
	...
	*/
	void make_deposit(int acct_number, double amt)
	{
		...
		if (acct) {
			...
			double NewAmount = acct->get_balance() + amt;
			acct->set_balance(NewAmount);
		}
	}
